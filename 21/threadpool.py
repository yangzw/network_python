...
def handleconnection(clientsock):
	...
	print "ok, we are appending clientsock to queue now."
        queue.append(clientsock)
        sem.release()
    finally:
        lockpool.release()
	print "relsease lockpool in handle"
...
def threadworker():
    ...
    print "ok, we are in threadworker now, first sleep for a while"
    time.sleep(1) # Simulate expensive startup
    name = currentThread().getName()
    try:
        lockpool.acquire()
	print "got lockpool in threadworker"
        try:
            waitinglist[name] = 1
        finally:
            lockpool.release()
...
def processclients():
    ...
    print "ok, now we are processing sclient now"
    name = currentThread().getName()
